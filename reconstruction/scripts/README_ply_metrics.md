...
`ply_metrics.py` 用来把不同模型输出的 `.ply` 点云放在同一套规则下计算指标，适合比较 baseline 3DGS、Scaffold-GS、GaussianPlant 等方法。
...
如果还要计算渲染图边缘强度，额外提供该模型的 test 渲染图目录：

```text
方法名或实验名|/path/to/point_cloud.ply|/path/to/renders/test
```

注意：这里用 `|` 分隔，不用冒号。这样 Windows 路径里的 `D:\...` 不会被误拆。
...
```bash
python reconstruction/scripts/ply_metrics.py \
...
```bash
/root/autodl-tmp/ml_course/envs/scaffold312/bin/python \
  /root/autodl-tmp/ml_course/reconstruction/scripts/ply_metrics.py \
  ...
  --out-csv /root/autodl-tmp/ml_course/compare_30k/scaffold_30k_ply_metrics.csv \
  --out-json /root/autodl-tmp/ml_course/compare_30k/scaffold_30k_ply_metrics.json
```
...
```powershell
python reconstruction\scripts\ply_metrics.py `
  --item "scaffold_001|D:\Sophomore_AIA\ML_course\compare_30k_results\compare_30k\scaffold\plant_001\model\point_cloud\iteration_30000\point_cloud.ply|D:\Sophomore_AIA\ML_course\compare_30k_results\compare_30k\scaffold\plant_001\renders\test" `
  --out-csv tmp\ply_metrics.csv `
  --out-json tmp\ply_metrics.json
```
